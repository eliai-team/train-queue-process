...
def train(training_id):
  ...
  try:
    return_code = subprocess.run(f"accelerate launch --config_file={accelerate_config_file} --num_cpu_threads_per_process=1 train_network.py --dataset_config={dataset_config_filepath} --config_file={training_config_filepath} --sample_prompts={prompt_filepath}", shell=True) 
